...
const defaultOutputRoot = resolve(root, "artifacts/plugin-layout");
...
const pluginFiles = [
  ...
  ["dist/hooks/entry.mjs", "dist/hooks/entry.mjs"],
  ["README.md", "README.md"],
...
export async function validatePluginLayout({
  ...
  const entryPath = resolve(outputRoot, "marketplace-entry.json");
  const marketplace = await readJson(marketplacePath, "plugin marketplace");
  const entry = await readJson(entryPath, "plugin marketplace entry");

  assert(
    Array.isArray(marketplace.plugins),
    "plugin marketplace plugins must be an array",
  );
  assert(
    marketplace.plugins.length === 1,
    "plugin marketplace fixture must contain one plugin",
  );
  assert(
    JSON.stringify(marketplace.plugins[0]) === JSON.stringify(entry),
    "plugin marketplace fixture and entry differ",
  );
  assert(
    entry.source === `./plugins/${entry.name}`,
    "plugin source must be ./plugins/<name>",
  );
  assert(
    entry.category === "developer-tools",
    "plugin category must be developer-tools",
  );
...
export async function assemblePluginLayout({
  sourceRoot = root,
  outputRoot = resolve(sourceRoot, "artifacts/plugin-layout"),
} = {}) {
  const resolvedSourceRoot = resolve(sourceRoot);
  const resolvedOutputRoot = resolve(outputRoot);
  assert(
    resolvedSourceRoot !== resolvedOutputRoot,
    "plugin layout output must not overwrite the source repository",
  );
  assert(
    isStrictChild(resolve(resolvedSourceRoot, "artifacts"), resolvedOutputRoot),
    "plugin layout output must be a child of the artifacts directory",
  );
  ...
  await ensureRegularFile(
    resolve(resolvedSourceRoot, "dist/hooks/entry.mjs"),
  ...
  await writeJson(resolve(pluginLayoutRoot, "package.json"), {
    name: manifest.name,
    version: manifest.version,
    description: manifest.description,
  ...
  await writeJson(resolve(resolvedOutputRoot, "marketplace-entry.json"), entry);
  await writeJson(
    resolve(resolvedOutputRoot, "marketplace.json"),
    marketplaceFixture(entry),
  );

  return validatePluginLayout({ outputRoot: resolvedOutputRoot });
}
...
async function main() {
  ...
  const sourceRoot = optionValue(args, "--source-root", root);
  const outputRoot = optionValue(
    args,
    "--output-root",
    resolve(sourceRoot, "artifacts/plugin-layout"),
  );
  const result = await assemblePluginLayout({ sourceRoot, outputRoot });
  process.stdout.write(
    `${JSON.stringify({
      plugin: result.name,
      version: result.version,
      output: relative(resolve(sourceRoot), result.outputRoot),
    })}\n`,
  );
}
...
if (
  ...
  try {
    ...
    const reason = error instanceof Error ? error.message : String(error);
    process.stderr.write(`build-layout failed: ${reason}\n`);
    process.exitCode = 1;
  }
}
