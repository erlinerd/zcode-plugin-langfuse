...
async function makeLayout(): Promise<string> {
  ...
  const distEntry = join(sourceRoot, "dist/hooks/entry.mjs");
  await mkdir(join(distEntry, ".."), { recursive: true });
  await writeFile(distEntry, "// Generated by npm run build\nexport {};\n");

  const outputRoot = join(sourceRoot, "artifacts", "plugin-layout");
