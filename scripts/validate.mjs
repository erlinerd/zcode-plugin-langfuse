...
const builtEntry = artifactMode
  ? fs.readFileSync(
      path.join(root, "dist/payload/dist/hooks/entry.mjs"),
      "utf8",
    )
  : null;
...
if (artifactMode) {
  assert(
    ...
    "dist/payload/dist/hooks/entry.mjs is not a current build output",
  );
}
...
if (artifactMode) {
  textFiles.push("dist/payload/dist/hooks/entry.mjs");
}
